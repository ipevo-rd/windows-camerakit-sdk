...
namespace IpevoSdkDemo
{
     ...
     *
     * This demo covers most of the IcCamera property APIs. Since every property follows
     * the same access pattern (HasCapability -> Get Min/Max/Delta/Current -> Set),
     * the UI rows are generated by a few shared helper methods, and each row simply
     * maps that pattern to a specific pair of IcCamera methods.
     *
     * When the selected camera does not support a feature, the row title is shown
     * in RED and its controls are disabled.
     *
     * Properties not shown in this demo (e.g., AiVoice, AiStage, InterLedFlash,
     * ButtonLock, InnerCamera, GetHighResolutionImage) can be used in exactly the same manner.
     *
     * ---
     *
